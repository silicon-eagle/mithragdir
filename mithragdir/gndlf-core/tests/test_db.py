...
class TestRedbookDatabase:
    ...
    def test_insert_document_returns_unique_ids(self, db: RedbookDatabase) -> None:
        id1 = db.insert_document(PageSchema(title='Doc 1', pageid=1, url='http://a.com', content='content a'))
        id2 = db.insert_document(PageSchema(title='Doc 2', pageid=2, url='http://b.com', content='content b'))
        assert id1 != id2
        assert id1 > 0
        assert id2 > 0
    ...
    def test_document_exists(self, db: RedbookDatabase) -> None:
        db.insert_document(PageSchema(title='Title', pageid=123, url='http://example.com', content='content'))
    ...
    def test_insert_document_stores_wiki_metadata(self, db: RedbookDatabase) -> None:
        ...
        wiki_page = WikiPage.get(WikiPage.document == document_id)
    ...
    def test_requires_postgres_database_url(self, monkeypatch: pytest.MonkeyPatch) -> None:
